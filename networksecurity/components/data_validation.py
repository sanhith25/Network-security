    ...
    def detect_dataset_drift(self, base_df, current_df, threshold=0.05) -> bool:
        try:
            ...
            for column in base_df.columns:
                ...
                test_result = ks_2samp(d1.dropna(), d2.dropna())
                if threshold <= test_result.pvalue:
                    is_found = False
                else:
                    is_found = True
                    status = False
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        try:
            ...
            if not self.validate_number_of_columns(train_dataframe):
                raise NetworkSecurityException("Train dataframe missing required columns", sys)
            if not self.validate_number_of_columns(test_dataframe):
                raise NetworkSecurityException("Test dataframe missing required columns", sys)
